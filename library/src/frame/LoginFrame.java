...
public class LoginFrame extends JFrame implements ItemListener {
	...
	public LoginFrame() {
		...
		loginButton = new JButton(new ImageIcon("D:\\Code\\Java_Code\\library\\image\\loginbutton.jpg"));
		...
		JLabel background1 = new JLabel(new ImageIcon("D:\\Code\\Java_Code\\library\\image\\Login.jpg"));
	...
	private void check_login() {
		if (this.readerLogin == true) {
			...
			reader = new Reader();
			reader.setReaderId(nameUserTextField.getText());
			...
			if (nameUserTextField.getText() != null && !"".equals(nameUserTextField.getText())
				...
				boolean whether_login = rTools.ReaderLogin(reader.getReaderId(), reader.getPassword());
				nameReader = rTools.ReaderDataById(reader.getReaderId()).get(0).getReaderName();
				if (whether_login == true) {
					idReader = reader.getReaderId();
		...
		if (this.librarianLogin == true) {
			...
			lib = new Librarian();
			lib.setUsername(nameUserTextField.getText());
			lib.setPassword(new String(passwordField.getPassword()));
			if (nameUserTextField.getText() != null && !"".equals(nameUserTextField.getText())
					&& passwordField.getPassword() != null && !("".equals(new String(passwordField.getPassword())))) {

				boolean whether_login = libTools.Login(lib.getUsername(), lib.getPassword());
				if (whether_login == true) {
					nameUser = lib.getUsername();
