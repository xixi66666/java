...
/*	public static void main(String[] args) {
	...
	private void show_data() {
		...
		if (idReadertextField.getText() != null && !"".equals(idReadertextField.getText())) {
			reader.setReaderId(idReadertextField.getText());
		} else {
			JOptionPane.showMessageDialog(this, "请输入读者编号", "", JOptionPane.WARNING_MESSAGE);
			return;
		}
		List<Reader> readerlist = readerTools.ReaderDataById(reader.getReaderId());
		List<Book> booklist = borrowtools.BookData(reader.getReaderId());
		...
		if (readerlist.size() == 0) {
			...
			for (Iterator<Reader> iterator = readerlist.iterator(); iterator.hasNext();) {
				...
				showNameReaderLabel.setText(temp.getReaderName());
			...
			for (Iterator<Book> iterator = booklist.iterator(); iterator.hasNext();) {
				...
				defaultModel.addRow(new Object[] { temp.getBookId(), temp.getBookName(), temp.getPrice() + "元",
